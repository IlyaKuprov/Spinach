...
- Lines 68-69: Calculate shot spacing; implemented by `parameters.shot_spacing=204e-6 - 2*parameters.nloops*parameters.pulse_dur`.
- Lines 71-72: Run the steady state simulation; implemented by `dnp=powder(spin_system,@xixdnp_steady,parameters,'esr')`.
- Lines 74-75: Plotting; implemented by `kfigure(); plot(parameters.el_offs/1e6,real(dnp))`.
- Lines 80-81: Save the figure; implemented by `savefig(gcf,'xix_q_field_profile_single.fig')`.
