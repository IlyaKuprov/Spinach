...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/fundamentals/correlation_function_1.m`
- Signature: `correlation_function_1()`
- Total lines: 96

## Purpose

Computes rotational correlation functions using a Monte-Carlo method and compares them to the analytical results returned by Spinach kernel for the following correlation function: G(L,k,m,p,q)=<D{L}(k,m)*D{L}(p,q)'> The sigma parameters refer to the rates of rotation and the four indices to the Wigner functions being correlated. Isotropic rotational diffusion tested here. Calculation time: minutes.
...
- Lines 17-18: Set testing parameters; implemented by `sigma_iso=0.1; L=2; k=-1; m=2; p=-1; q=2`.
...
- Lines 40-43: Generate and apply a random rotation; implemented by `R_gen=sigma_iso*[ 0 1 0; -1 0 0; 0 0 0]*angles(1,n)+ sigma_iso*[ 0 0 1; 0 0 0; -1 0 0]*angles(2,n)+ sigma_iso*[ 0 0 0; 0 0 1; 0 -1 0]*angles(3,n)`.
...
- Lines 18: computes `sigma_iso` using `sigma_iso=0.1; L=2; k=-1; m=2; p=-1; q=2`.
...
- Lines 41-43: computes `R_gen` using `R_gen=sigma_iso*[ 0 1 0; -1 0 0; 0 0 0]*angles(1,n)+ sigma_iso*[ 0 0 1; 0 0 0; -1 0 0]*angles(2,n)+ sigma_iso*[ 0 0 0; 0 0 1; 0 -1 0]*angles(3,n)`.
...
- Lines 75: computes `inter.tau_c` using `inter.tau_c={1/(3*sigma_iso^2)}`.
...
- to the Wigner functions being correlated. Isotropic rotational diffusion
- tested here.
