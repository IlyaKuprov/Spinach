...
For cooperative phase control, `grape_coop` retains the requested primary fidelity and uses real-overlap auxiliary derivatives for its squared-impurity penalty, including with nonunit targets. Zero real auxiliary overlaps and zero impurity gradients are valid engine outputs. For all four methods (`lbfgs`, `rbfgs`, `newton`, and `goodwin`), `fmaxnewton` checks the initial assembled gradient on unfrozen coordinates before constructing a search direction; do not reject individual auxiliary contributions. The initial gradient norm must be at least `1e-6`; ordinary terminal objectives without trajectory penalties also require their first fidelity channel to have magnitude at least `1e-6`. `grape_coop` is exempt from that value check because its first channel includes the impurity penalty and can cancel a substantial primary transfer; the assembled-gradient check still applies. Objective-only calls with `max_iter=0` do not apply these initial-guess checks.
