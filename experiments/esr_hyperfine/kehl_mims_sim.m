...
function [endor_amp,endor_amp_conv,x_coords,v_L]=kehl_mims_sim(constants,spinSys,spinOps,expt,opt)
    ...
    if opt("Relax")==true
        ...
        endor_amp=kehl_mims_rlx(constants,spinOps,spinSys,expt,opt,paramsENDOR,epr);

    else

        % starts the actual calculation
        endor_amp=kehl_mims_calc(constants,spinOps,spinSys,expt,opt,paramsENDOR,epr);
    end
