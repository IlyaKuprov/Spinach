...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/steady_state/tppm_q_rep_time_ensemble_b1_r.m`
- Signature: `tppm_q_rep_time_ensemble_b1_r()`
- Total lines: 120

## Purpose

Simulation of TPPM DNP repetition time scan in the steady state with distributions in electron-proton distance and microwave B1 field. Calculation time: hours.
...
- Lines 40-41: Log spacing for rep. time; implemented by `rep_time=logspace(-5.0,-2.7,30)`.
- Lines 43-44: Preallocate equilibrium DNP value array; implemented by `dnp=zeros([numel(rep_time) numel(r) numel(b1)],'like',1i)`.
...
- Line 86: `parfor` loop over `m=1:numel(rep_time)`.
...
- Lines 38: computes `[b1,wb1]` using `[b1,wb1]=gaussleg(25e6,35e6,5)`.
- Lines 41: computes `rep_time` using `rep_time=logspace(-5.0,-2.7,30)`.
- Lines 44: computes `dnp` using `dnp=zeros([numel(rep_time) numel(r) numel(b1)],'like',1i)`.
