...
- Lines 17-18: Set testing parameters; implemented by `sigma_iso=0.2; L=4; k=-1; m=2; p=-1; q=2`.
- Lines 20-21: Convert indices from [-L,L] to [1,2*L+1]; implemented by `k=L+1+k; m=L+1+m; p=L+1+p; q=L+1+q`.
- Lines 23-26: % Numerical Monte-Carlo calculation; implemented by `npoints=1e6; nlags=100`.
- Lines 25-26: Number of points and lags; implemented by `npoints=1e6; nlags=100`.
- Lines 28-29: Generate angle track; implemented by `angles=randn(3,npoints)`.
...
- Lines 40-43: Generate a random rotation; implemented by `R_gen=sigma_iso*[ 0 1 0; -1 0 0; 0 0 0]*angles(1,n)+ sigma_iso*[ 0 0 1; 0 0 0; -1 0 0]*angles(2,n)+ sigma_iso*[ 0 0 0; 0 0 1; 0 -1 0]*angles(3,n)`.
...
- Lines 18: computes `sigma_iso` using `sigma_iso=0.2; L=4; k=-1; m=2; p=-1; q=2`.
- Lines 21: computes `k` using `k=L+1+k; m=L+1+m; p=L+1+p; q=L+1+q`.
- Lines 26: computes `npoints` using `npoints=1e6; nlags=100`.
