...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/nmr_liquids/noe_two_spin_het.m`
...
- Lines 10-11: Set the spin system; implemented by `sys.isotopes={'1H','13C'}`.
...
- Lines 40-41: Start in a state with proton spin inverted; implemented by `Lz1=state(spin_system,{'Lz'},{1})`.
- Lines 44-46: Compute the evolution trajectory; implemented by `coil=[state(spin_system,{'Lz'},{1}) state(spin_system,{'Lz'},{2})]`.
- Lines 49-50: Plot the longitudinal magnetization; implemented by `kfigure(); plot(linspace(0,4,401),answer)`.

### Key state/data transformations

- Lines 11: computes `sys.isotopes` using `sys.isotopes={'1H','13C'}`.
...
- Lines 47: computes `answer` using `answer=evolution(spin_system,1i*R,coil,rho,1e-2,400,'multichannel')`.
