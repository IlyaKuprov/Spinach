...
- Lines 41-42: Spectrum setup; implemented by `parameters.max_rank=8`.
- Lines 62-63: Simulation; implemented by `spectrum=singlerot(spin_system,@overtone_pa,parameters,'qnmr')`.
- Lines 65-66: Phasing; implemented by `spectrum=exp(1i*1.75)*spectrum`.
...
- Lines 42: computes `parameters.max_rank` using `parameters.max_rank=8`.
- Lines 43: computes `parameters.axis` using `parameters.axis=[sqrt(2/3) 0 sqrt(1/3)]`.
- Lines 44: computes `parameters.rate` using `parameters.rate=-19840`.
- Lines 45: computes `parameters.grid` using `parameters.grid='rep_2ang_6400pts_sph'`.
- Lines 46: computes `parameters.sweep` using `parameters.sweep=[75e3 100e3]`.
