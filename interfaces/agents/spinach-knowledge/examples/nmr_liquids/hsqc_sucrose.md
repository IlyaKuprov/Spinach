...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/nmr_liquids/hsqc_sucrose.m`
...
- Lines 64-65: Simulation; implemented by `fid=liquid(subsystem,@hsqc,parameters,'nmr')`.
- Lines 67-68: Apodisation; implemented by `fid.pos=apodisation(spin_system,fid.pos,{{'cos'},{'cos'}})`.
- Lines 71-72: F2 Fourier transform; implemented by `f1_pos=fftshift(fft(fid.pos,parameters.zerofill(2),1),1)`.
- Lines 75-76: Form States signal; implemented by `fid=f1_pos+conj(f1_neg)`.
- Lines 78-79: F1 Fourier transform; implemented by `spectrum=spectrum+fftshift(fft(fid,parameters.zerofill(1),2),2)`.
...
- Lines 39: computes `parameters.sweep` using `parameters.sweep=[3350 950]`.
- Lines 40: computes `parameters.offset` using `parameters.offset=[5000 1100]`.
