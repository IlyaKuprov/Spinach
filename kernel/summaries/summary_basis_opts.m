...
if strcmp(spin_system.bas.formalism,'sphten-liouv')
    switch spin_system.bas.approximation
        case 'IK-0'
            report(spin_system,['IK-0 approximation - all correlations of all spins up to order ' int2str(spin_system.bas.inter_level)...
                                ' within each chemical substance.']);
        case 'IK-1'
            report(spin_system,['IK-1 approximation - spin correlations up to order ' int2str(spin_system.bas.inter_level)...
                                ' between directly coupled spins.']);
            report(spin_system,['IK-1 approximation - spin correlations up to order ' int2str(spin_system.bas.prox_level)...
                                ' between all spins within ' num2str(spin_system.tols.prox_cutoff) ' Angstrom of each other.']);
            report(spin_system,['IK-1 approximation - coupling graph from ' strrep(spin_system.bas.connectivity,'_',' ') ...
        ...
        case 'IK-2'
            ...
            report(spin_system,['IK-2 approximation - spin correlations up to order ' int2str(spin_system.bas.prox_level)...
                                ' between all spins within ' num2str(spin_system.tols.prox_cutoff) ' Angstrom of each other.']);
            report(spin_system,['IK-2 approximation - coupling graph from ' strrep(spin_system.bas.connectivity,'_',' ') ...
                                ', tensors with 2-norm below ' num2str(spin_system.tols.inter_cutoff) ' Hz dropped.']);
        case 'IK-DNP'
