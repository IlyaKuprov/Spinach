...
- Lines 46-47: Simulation; implemented by `fid=liquid(spin_system,@hnca,parameters,'nmr')`.
...
- Lines 61-62: Absorption part of F3 signal; implemented by `f3_pos=f3_pos_pos+conj(f3_neg_neg)`.
- Lines 65-66: F2 Fourier transform; implemented by `f3f2_pos=fftshift(fft(f3_pos,parameters.zerofill(2),2),2)`.
- Lines 69-70: Absorption part of F2 signal; implemented by `f3f2=f3f2_pos+conj(f3f2_neg)`.
- Lines 72-73: F1 Fourier transform; implemented by `spectrum=fftshift(fft(f3f2,parameters.zerofill(1),1),1)`.
- Lines 75-77: Plotting; implemented by `kfigure(); plot_3d(spin_system,-real(spectrum),parameters, 10,[0.1 0.5 0.1 0.5],2,'positive')`.
...
- Lines 40: computes `parameters.sweep` using `parameters.sweep=[2800 5000 3000]`.
- Lines 41: computes `parameters.offset` using `parameters.offset=[-7200 8600 5100]`.
- Lines 42: computes `parameters.npoints` using `parameters.npoints=[128 128 128]`.
