...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/cross_effect_freq_scan_3.m`
- Signature: `cross_effect_freq_scan_3()`
- Total lines: 72

## Purpose

A simple TOTAPOL based Cross Effect DNP system. Set to repro- duce Figure 2b from Intensity differences are due to a different relaxation model and minor inconsistencies between the stated geometry and the interaction amplitudes used in the original paper. Electron rotating frame simulation using Nottingham DNP rela- xation theory detailed in Calculation time: seconds
...
- Lines 66-67: Plotting; implemented by `kfigure(); plot(linspace(-350,350,1e4),real(answer)); kgrid`.

### Key state/data transformations

- Lines 24: computes `sys.magnet` using `sys.magnet=3.4`.
- Lines 27: computes `sys.isotopes` using `sys.isotopes={'E','E','1H'}`.
- Lines 28: computes `inter.zeeman.scalar` using `inter.zeeman.scalar={2.0023193 1.9992887 0.0000000}`.
...
- Lines 54: computes `parameters.mw_frq` using `parameters.mw_frq=2*pi*linspace(-350,350,1e4)*1e6`.
