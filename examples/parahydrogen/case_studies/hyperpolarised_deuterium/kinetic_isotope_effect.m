...
function kinetic_isotope_effect() 
...
[S,T,Q,Tc,Qc]=deut_pair(spin_system,1,2);
...
kfigure(); scale_figure([1.75 0.75]); subplot(1,2,1);
...
% Assemble the coils array
coils=[Tc{2} Qc{3} Qc{4}]; coils(1,:)=0;

% 45-degree pulse on deuterium
Dy=operator(spin_system,'Ly','2H');
traj=step(spin_system,Dy,[traj_a traj_b],pi/4);

% Observables
obs=coils'*traj;

% Plotting
subplot(1,2,2); plot(time_axis',real(obs)');
kxlabel('time, seconds'); xlim tight; ylim padded;
kylabel('spin state coefficient, a.u.'); kgrid;
klegend('$T_{1} \rightarrow T_{0}$',...
        '$Q_{1} \rightarrow Q_{0}$',...
