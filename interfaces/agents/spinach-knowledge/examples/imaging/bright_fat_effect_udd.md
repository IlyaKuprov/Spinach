...
- Lines 30-31: Spins 1,2,3 are molecule A; spins 4,5,6 are molecule B; implemented by `inter.chem.parts={[1 2 3],[4 5 6]}`.
- Lines 33-34: Kinetic rate matrix (Hz); implemented by `inter.chem.rates=[0 0; 0 0]`.
- Lines 37-38: Basis set; implemented by `bas.formalism='sphten-liouv'`.
- Lines 41-42: Disable path tracing; implemented by `sys.disable={'pt'}`.
- Lines 44-48: This needs a GPU sys.enable={'gpu'};; implemented by `spin_system=create(sys,inter)`.
- Lines 47-48: Spinach housekeeping; implemented by `spin_system=create(sys,inter)`.
- Lines 51-52: Sequence parameters; implemented by `parameters.spins={'1H'}`.
- Lines 58-59: Sample geometry; implemented by `parameters.dims=[0.30 0.25]`.
- Lines 63-64: Relaxation phantoms and operators; implemented by `parameters.rlx_ph={}; parameters.rlx_op={}`.
- Lines 66-67: Initial and detection state phantoms; implemented by `load('../../etc/phantoms/bright_fat_left.mat','left')`.
- Lines 75-76: No diffusion or flow; implemented by `parameters.u=zeros(parameters.npts)`.
- Lines 80-81: Run the simulation; implemented by `mri=imaging(spin_system,@udd_dec,parameters)`.
- Lines 83-84: Plotting; implemented by `kfigure(); surf(abs(mri)); set(gca,'XDir','reverse')`.
...
- Lines 31: computes `inter.chem.parts` using `inter.chem.parts={[1 2 3],[4 5 6]}`.
- Lines 34: computes `inter.chem.rates` using `inter.chem.rates=[0 0; 0 0]`.
- Lines 35: computes `inter.chem.concs` using `inter.chem.concs=[1 1]`.
...
- Lines 42: computes `sys.disable` using `sys.disable={'pt'}`.
