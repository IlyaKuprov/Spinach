...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_liq/ccdnp/freq_scan_si_sys_b.m`
- Signature: `freq_scan_si_sys_b()`
- Total lines: 105
...
- Lines 30-31: coordinates; implemented by `inter.coordinates={[ 0.000 0.000 0.000]`.
...
- Lines 60-61: Microwave frequency offset grid; implemented by `parameters.mw_frq=2*pi*linspace(-15,15,512)*1e6`.
- Lines 63-64: Magnetic field grid, Tesla; implemented by `field_grid=linspace(1,30,128)`.
...
- Lines 21: computes `inter.zeeman.eigs{1,2}` using `inter.zeeman.eigs{1,2}=[1.9778 1.9776 1.9776]`.
- Lines 22: computes `inter.zeeman.euler{1,2}` using `inter.zeeman.euler{1,2}=[-0.180 0.017 0.194]`.
- Lines 23: computes `inter.zeeman.eigs{1,3}` using `inter.zeeman.eigs{1,3}=[2.0068 2.0038 2.0038]`.
- Lines 24: computes `inter.zeeman.euler{1,3}` using `inter.zeeman.euler{1,3}=[0.632 0.783 1.086]`.
- Lines 27: computes `inter.coupling.scalar` using `inter.coupling.scalar=cell(3,3)`.
- Lines 28: computes `inter.coupling.scalar{2,3}` using `inter.coupling.scalar{2,3}=5e6`.
