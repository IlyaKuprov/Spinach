...
- Lines 76-77: Hard pulses sliced at the drift tick interval; implemented by `hard_pulses=cell(1,3)`.
- Lines 84-85: Optimal control pulse sequence; implemented by `exc=load(['mq_exc_' num2str(mq_order) 'q.mat'],'pulse','pulse_dt')`.
- Lines 90-91: Loop over the two sequences; implemented by `sequences={hard_pulses,oc_pulses}; efficiency=zeros(1,2)`.
- Lines 94-95: Parallel loop over the ensemble; implemented by `signals=zeros(1,numel(drifts)); pulses=sequences{s}`.
- Lines 98-99: Start with the initial state at time zero; implemented by `rho=rho_init; current_time=0`.
- Lines 101-102: Loop over the three pulses; implemented by `for k=1:3`.
- Lines 104-105: Loop over the slices of the pulse; implemented by `for m=1:numel(pulses{k}{2})`.
- Lines 107-108: Drift Hamiltonian at the slice midpoint; implemented by `tick_idx=floor((current_time+pulses{k}{2}(m)/2)/tick_dt)+1`.
- Lines 110-113: Take a time step; implemented by `rho=step(spin_system,drifts{n}{tick_idx}+ pulses{k}{1}(1,m)*Lx+pulses{k}{1}(2,m)*Ly, rho,pulses{k}{2}(m))`.

### Control flow inferred from the code

- Line 69: conditional branch on `mq_order==3`.
- Line 78: `for` loop over `k=1:3`.
- Line 92: `for` loop over `s=1:2`.
- Line 96: `parfor` loop over `n=1:numel(drifts)`.
- Line 102: `for` loop over `k=1:3`.
- Line 105: `for` loop over `m=1:numel(pulses{k}{2})`.
- Line 119: conditional branch on `k==1`.
