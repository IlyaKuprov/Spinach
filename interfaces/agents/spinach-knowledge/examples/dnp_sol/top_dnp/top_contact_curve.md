...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/top_dnp/top_contact_curve.m`
...
- Lines 52-53: Run the calculation; implemented by `contact_curve=powder(spin_system,@topdnp,parameters,'esr')`.
- Lines 55-58: Plotting; implemented by `time_axis=linspace(0,parameters.nloops* (parameters.pulse_dur+parameters.delay_dur), parameters.nloops+1)`.
...
- Lines 44: computes `parameters.offset` using `parameters.offset=[(-13.0+92.5)*1e6 0]`.
- Lines 45: computes `parameters.irr_powers` using `parameters.irr_powers=17.8e6`.
- Lines 46: computes `parameters.pulse_dur` using `parameters.pulse_dur=10e-9`.
- Lines 47: computes `parameters.delay_dur` using `parameters.delay_dur=14e-9`.
- Lines 48: computes `parameters.nloops` using `parameters.nloops=300`.
- Lines 49: computes `parameters.grid` using `parameters.grid='rep_2ang_3200pts_sph'`.
