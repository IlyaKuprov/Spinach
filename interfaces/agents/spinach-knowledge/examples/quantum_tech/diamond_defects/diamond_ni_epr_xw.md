...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/quantum_tech/diamond_defects/diamond_ni_epr_xw.m`
...
- Lines 10-11: Set Ni NE1 centre model parameters.; implemented by `ni_params.centre='ne1'`.
- Lines 15-16: Build the spin system; implemented by `[sys,inter]=diamond_ni(ni_params)`.
...
- Lines 11: computes `ni_params.centre` using `ni_params.centre='ne1'`.
- Lines 12: computes `ni_params.orientation` using `ni_params.orientation='111'`.
- Lines 13: computes `ni_params.nickel` using `ni_params.nickel='61Ni'`.
- Lines 16: computes `[sys,inter]` using `[sys,inter]=diamond_ni(ni_params)`.
...
- Lines 32: computes `parameters.fwhm` using `parameters.fwhm=0.0001`.
...
- Lines 40: computes `parameters.window` using `parameters.window=[0.3 0.36]`.
...
- Called routines detected from the main body: `diamond_ni()`, `create()`, `basis()`, `fieldsweep()`, `kfigure()`, `scale_figure()`, `subplot()`, `kxlabel()`, `kylabel()`, `ktitle()`.
