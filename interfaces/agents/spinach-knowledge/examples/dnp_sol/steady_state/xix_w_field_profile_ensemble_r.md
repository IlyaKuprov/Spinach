...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/steady_state/xix_w_field_profile_ensemble_r.m`
...
- Lines 38-39: Microwave resonance offsets, Hz; implemented by `offsets=linspace(-300e6,300e6,201)`.
...
- Lines 76-77: Run the steady state simulation; implemented by `dnp(:,n)=powder(spin_system,@xixdnp_steady,parameters,'esr')`.
- Lines 81-82: Integrate over the distance distribution, r^2 is the Jacobian; implemented by `dnp=sum(dnp.*reshape(r.^2,[1 numel(r)]).*reshape(w,[1 numel(w)]),2)/sum((r.^2).*w)`.
- Lines 84-85: Plotting; implemented by `kfigure(); plot(parameters.el_offs/1e6,real(dnp))`.
...
- Lines 39: computes `offsets` using `offsets=linspace(-300e6,300e6,201)`.
