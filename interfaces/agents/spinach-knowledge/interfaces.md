...
|---|---|---|---:|
| `interfaces/b2spinach.m` | `bdata=b2spinach(inpath)` | Imports time-domain NMR data recorded by Bruker instruments: reads the binary fid or ser file together with the acquisit | 482 |
...
| `interfaces/comsol/conc_plot.m` | `conc_plot(spin_system,conc,obs)` | 2D microfluidic concentration plotting function. Uses mesh tessellation information to plot concentrations as vertical b | 206 |
...
| `interfaces/nmrpipe/fid2pipe.m` | `fid2pipe(spin_system,file_root,fid,parameters,nmrpipe_root)` | Exports phase-sensitive 2D Spinach free induction decays into native NMRPipe time-domain files. Syntax: fid2pipe(spin_sy | 203 |
...
| `interfaces/pdb_bmrb/protein.m` | `[sys,inter,aux]=protein(pdb_file,bmrb_file,options)` | Protein data import function. Parses PDB and BMRB data, runs a J-coupl- ing guess, a CSA guess and outputs Spinach data  | 524 |
...
| `interfaces/pdb_bmrb/read_pdb_nuc.m` | `[res_num,res_typ,pdb_id,coords]=read_pdb_nuc(pdb_file_name)` | Reads the coordinates of all atoms from the user-specified PDB file and returns, for each atom, the residue number, the  | 110 |
| `interfaces/pdb_bmrb/read_pdb_pro.m` | `[aa_num,aa_typ,pdb_id,coords,pdb_ser]=read_pdb_pro(pdb_file_name,mod_id)` | Reads the a PDB file and returns amino acid numbers, amino acid types, PDB atom identifiers and Cartesian coordinates. S | 120 |
...
| `interfaces/spinjet/awg_interface.m` | `data=awg_interface(spin_system,awg_cmd,cmd_input)` | Interface to the Bruker SpinJet AWG, calling a library of Python scripts that inteface with the Bruker Xepr API. Syntax: | 144 |
| `interfaces/spinjet/py_run.m` | `arg_out=py_run(spin_system,pyscript,arg_in)` | Runs a python script from /interfaces/spinjet/Xepr_python/ folder of Bruker Xepr installation. Variable inputs and outpu | 129 |
...
| `interfaces/spinxml/x2spinach.m` | `[sys,inter]=x2spinach(filename,shielding_refs)` | Reads SpinXML files and forms Spinach data structures. Syntax: [sys,inter]=x2spinach(filename,shielding_refs) Parameters | 899 |
