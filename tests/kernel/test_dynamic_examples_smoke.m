...
result=test_close(result,'CT-COSY FID norm',norm(fid(:),2),5.6305493030431162e+00,1e-10,1e-10,...
                  'the compact CT-COSY time-domain matrix is deterministic');
result=test_close(result,'CT-COSY spectrum norm',norm(spectrum(:),2),9.0088788848689859e+01,1e-10,1e-10,...
                  'the compact CT-COSY spectrum norm is deterministic');
result=test_close(result,'CT-COSY spectrum sum',sum(abs_spectrum(:)),6.3367922545678312e+02,1e-9,1e-10,...
                  'the compact CT-COSY absolute spectrum sum is deterministic');
result=test_close(result,'CT-COSY spectrum maximum',max(abs_spectrum(:)),3.1850510896725737e+01,1e-10,1e-10,...
