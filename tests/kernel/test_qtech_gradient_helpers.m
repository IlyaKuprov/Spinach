...
% Build a simple affine field map on a cube
[x,y,z]=ndgrid([0 1],[0 1],[0 1]);
map=[x(:) y(:) z(:) ...
     2*x(:)+3*y(:)+5*z(:) ...
    -1*x(:)+7*y(:) ...
     11*z(:)];
field_maps.coil_a=map;

% Interpolate it between grid points
positions=[0.25 0.50 0.75; 0.75 0.25 0.50];
fields=gradient_fields_at_spins(field_maps,positions,2,{'coil_a'});
