...
- Lines 10-11: Set NV centre parameters; implemented by `nv_params.orientation='111'`.
- Lines 14-15: Build the spin system; implemented by `[sys,inter]=diamond_nvm_gs(nv_params)`.
...
- Lines 28-29: EPR sim parameters; implemented by `parameters.spins={'E3'}`.
...
- Lines 11: computes `nv_params.orientation` using `nv_params.orientation='111'`.
- Lines 12: computes `nv_params.nitrogen` using `nv_params.nitrogen='14N'`.
- Lines 15: computes `[sys,inter]` using `[sys,inter]=diamond_nvm_gs(nv_params)`.
...
- Lines 31: computes `parameters.fwhm` using `parameters.fwhm=0.001`.
- Lines 32: computes `parameters.int_tol` using `parameters.int_tol=0.0001`.
...
- Lines 39: computes `parameters.window` using `parameters.window=[0.1 0.5]`.
