...
- Lines 13-14: Number of water protons; implemented by `n_water_protons=100`.
...
- Lines 29-30: Scalar couplings; implemented by `inter.coupling.scalar=cell(n_water_protons+4)`.
- Lines 34-35: Estimated relaxation times, seconds; implemented by `T1H=0.2722; T1N=0.8; T1Ca=2; T1CO=2; T1W=0.2994`.
- Lines 38-39: Relaxation theory; implemented by `inter.relaxation={'t1_t2'}`.
- Lines 46-47: Basis set; implemented by `bas.formalism='sphten-liouv'`.
- Lines 53-54: Exchange rates; implemented by `nh_wt_exch_rate=1000`.
- Lines 57-58: Exchange rate matrix; implemented by `inter.chem.flux_rate=zeros(n_water_protons+4)`.
- Lines 65-66: Spinach housekeeping; implemented by `spin_system=create(sys,inter)`.
- Lines 69-70: Simulation parameters; implemented by `parameters.spins={'1H','15N','13C'}`.
- Lines 75-76: Get system trajectory (sequence is below); implemented by `traj=liquid(spin_system,@frydman_pump,parameters,'nmr')`.
- Lines 78-79: Observables: peptide bond spins; implemented by `Hz=state(spin_system,{'Lz'},{1})`.
- Lines 86-87: Project out the observables; implemented by `Hz=real(Hz'*traj); Hx=real(Hx'*traj)`.
- Lines 91-92: Plotting; implemented by `kfigure(); scale_figure([1.0 2.0])`.

### Key state/data transformations

- Lines 14: computes `n_water_protons` using `n_water_protons=100`.
...
- Lines 32: computes `inter.coupling.scalar{2,4}` using `inter.coupling.scalar{2,4}=8`.
- Lines 35: computes `T1H` using `T1H=0.2722; T1N=0.8; T1Ca=2; T1CO=2; T1W=0.2994`.
- Lines 36: computes `T2H` using `T2H=0.2722; T2N=0.8; T2Ca=2; T2CO=2; T2W=0.2994`.
- Lines 39: computes `inter.relaxation` using `inter.relaxation={'t1_t2'}`.
- Lines 40: computes `inter.r1_rates` using `inter.r1_rates=num2cell(1./[T1H T1N T1Ca T1CO T1W*ones(1,n_water_protons)])`.
- Lines 41: computes `inter.r2_rates` using `inter.r2_rates=num2cell(1./[T2H T2N T2Ca T2CO T2W*ones(1,n_water_protons)])`.
- Lines 42: computes `inter.rlx_keep` using `inter.rlx_keep='diagonal'`.
...
- Line 109: `frydman_pump()` — `function traj=frydman_pump(spin_system,parameters,~,R,K)`. Get pulse operators
