...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/novel_dnp/novel_contact_curve.m`
...
- Lines 53-54: Run the calculation; implemented by `contact_curve=powder(spin_system,@noveldnp,parameters,'esr')`.
- Lines 56-57: Plotting; implemented by `time_axis=linspace(0,parameters.timestep*parameters.nsteps,parameters.nsteps+1)`.
...
- Lines 44: computes `parameters.offset` using `parameters.offset=[(-3.3+0.0)*1e6 0]`.
- Lines 45: computes `parameters.irr_powers` using `parameters.irr_powers=14.48e6`.
- Lines 46: computes `parameters.pulse_dur` using `parameters.pulse_dur=1/(4*parameters.irr_powers)`.
- Lines 47: computes `parameters.timestep` using `parameters.timestep=1e-9`.
- Lines 48: computes `parameters.grid` using `parameters.grid='rep_2ang_400pts_sph'`.
- Lines 49: computes `parameters.nsteps` using `parameters.nsteps=2400`.
