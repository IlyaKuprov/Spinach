...
Distributed fit for the S50C mutant dataset for human carbonic anhydrase II. The system and the method are described in: A step-by-step tutorial is available here:
...
- Lines 47-48: Get the new susceptibility tensor; implemented by `[chi,~]=chi_eff(source_cube,ranges,parameters.xyz,parameters.expt_pcs)`.
...
- Lines 30: computes `parameters.box_cent` using `parameters.box_cent=[-27.4 13.3 18.8]`.
...
- Lines 43: computes `[source_cube,ranges]` using `[source_cube,ranges]=ipcs(parameters,n,0.23)`.
- Lines 44: computes `parameters.guess` using `parameters.guess=source_cube`.
- Lines 48: computes `[chi,~]` using `[chi,~]=chi_eff(source_cube,ranges,parameters.xyz,parameters.expt_pcs)`.
