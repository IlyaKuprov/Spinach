...
- Lines 21-22: Current Y axis ruler; implemented by `ax=gca; ruler=ax.YAxis`.
- Lines 24-25: Check consistency; implemented by `grumble(ruler)`.
- Lines 27-28: Preserve any existing limit-change callback; implemented by `app_key='SpinachKYTickFix'`.
...
- Lines 22: computes `ax` using `ax=gca; ruler=ax.YAxis`.
- Lines 28: computes `app_key` using `app_key='SpinachKYTickFix'`.
...
- Line 100: `grumble()` — `function grumble(ruler)`. "Times are bad. Children no longer obey their parents and everyone is
  - Representative operation: `if ~isa(ruler,'matlab.graphics.axis.decorator.NumericRuler')`.
  - Representative operation: `error('Y axis must be numeric.')`.
