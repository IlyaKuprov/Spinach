...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/steady_state/novel_x_rep_time_single.m`
- Signature: `novel_x_rep_time_single()`
- Total lines: 114

## Purpose

Simulation of NOVEL DNP repetition time scan in the steady state. Calculation time: minutes.
...
- Lines 12-13: X-band magnet; implemented by `sys.magnet=0.34`.
...
- Lines 69-70: Log spacing for rep. time; implemented by `rep_time=logspace(-4,-2,30)`.
- Lines 72-73: Preallocate equilibrium DNP value arrays; implemented by `dnp_noflip=zeros(size(rep_time),'like',1i)`.
- Lines 76-77: Over repetition times; implemented by `parfor m=1:numel(rep_time)`.
- Lines 79-80: Localise parameters; implemented by `localpar=parameters`.
- Lines 82-83: Set the shot spacing, no flipback pulse; implemented by `localpar.flipback=0`.

### Control flow inferred from the code

- Line 77: `parfor` loop over `m=1:numel(rep_time)`.

### Key state/data transformations

- Lines 13: computes `sys.magnet` using `sys.magnet=0.34`.
...
- Lines 45-46: computes `r1n_rate` using `r1n_rate=@(alp,bet,gam)r1n_dnp(sys.magnet,inter.temperature, 2.00230,1e-3,26,r_en,bet)`.
- Lines 47: computes `inter.r1_rates` using `inter.r1_rates={1e3 r1n_rate}`.
- Lines 48: computes `inter.r2_rates` using `inter.r2_rates={200e3 50e3}`.
...
- Called routines detected from the main body: `cell2mat()`, `xyz()`, `r1n_dnp()`, `create()`, `basis()`, `state()`, `rep_time()`, `dnp_noflip()`, `powder()`, `dnp_flip()`, `kfigure()`, `kylabel()`, `klegend()`, `kxlabel()`, `xlim()`, `savefig()`.
