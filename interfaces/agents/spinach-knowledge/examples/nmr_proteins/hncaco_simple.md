...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/nmr_proteins/hncaco_simple.m`
...
- Lines 17-18: Interactions; implemented by `inter.zeeman.scalar={110 60 7 180}`.
...
- Lines 35-36: Sequence parameters; implemented by `parameters.J_nh=92`.
- Lines 46-47: Simulation; implemented by `fid=liquid(spin_system,@hncaco,parameters,'nmr')`.
- Lines 49-50: Apodisation; implemented by `fid.pos_pos=apodisation(spin_system,fid.pos_pos,{{'sqcos'},{'sqcos'},{'sqcos'}})`.
- Lines 55-56: F3 Fourier transform; implemented by `f3_pos_pos=fftshift(fft(fid.pos_pos,parameters.zerofill(3),3),3)`.
- Lines 61-62: Absorption part of F3 signal; implemented by `f3_pos=f3_pos_pos+1i*conj(f3_neg_neg)`.
- Lines 65-66: F2 Fourier transform; implemented by `f3f2_pos=fftshift(fft(f3_pos,parameters.zerofill(2),2),2)`.
- Lines 69-70: Absorption part of F2 signal; implemented by `f3f2=f3f2_pos+1i*conj(f3f2_neg)`.
- Lines 72-73: F1 Fourier transform; implemented by `spectrum=fftshift(fft(f3f2,parameters.zerofill(1),1),1)`.
- Lines 75-77: Plotting; implemented by `kfigure(); plot_3d(spin_system,real(spectrum),parameters, 10,[0.2 0.9 0.2 0.9],2,'positive')`.
...
- Lines 18: computes `inter.zeeman.scalar` using `inter.zeeman.scalar={110 60 7 180}`.
...
- Lines 36: computes `parameters.J_nh` using `parameters.J_nh=92`.
- Lines 37: computes `parameters.T` using `parameters.T=25e-3`.
- Lines 38: computes `parameters.delta2` using `parameters.delta2=3e-3`.
