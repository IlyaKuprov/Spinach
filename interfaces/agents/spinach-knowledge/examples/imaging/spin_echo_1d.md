...
- Lines 40-41: Relaxation phantom; implemented by `parameters.rlx_ph={zeros(parameters.npts,1)}`.
...
- Lines 54-55: Run the simulation; implemented by `echo=imaging(spin_system,@spin_echo,parameters)`.
- Lines 57-58: Plotting; implemented by `time_axis=(0:400)*parameters.g_step_dur`.
...
- Lines 41: computes `parameters.rlx_ph` using `parameters.rlx_ph={zeros(parameters.npts,1)}`.
- Lines 42: computes `parameters.rlx_op` using `parameters.rlx_op={relaxation(spin_system)}`.
