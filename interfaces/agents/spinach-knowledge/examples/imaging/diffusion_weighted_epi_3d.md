...
- Lines 71-72: Phantom library call; implemented by `[R1_Ph,R2_Ph,PD_Ph,dims,npts]=phantoms('brain-medres')`.
- Lines 74-75: Sample settings; implemented by `parameters.dims=dims`.
- Lines 81-82: Relaxation phantom; implemented by `[R1,R2]=rlx_t1_t2(spin_system)`.
...
- Lines 24: computes `inter.r1_rates` using `inter.r1_rates={1.0}`.
- Lines 25: computes `inter.r2_rates` using `inter.r2_rates={1.0}`.
