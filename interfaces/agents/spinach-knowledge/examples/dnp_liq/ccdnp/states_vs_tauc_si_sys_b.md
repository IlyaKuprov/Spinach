...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_liq/ccdnp/states_vs_tauc_si_sys_b.m`
...
- Lines 57-58: Correlation time grid; implemented by `tau_c=linspace(50e-12,500e-12,64)`.
- Lines 60-61: Preallocate results; implemented by `answer=zeros(12,numel(tau_c),'like',1i)`.
- Lines 63-64: Disable excessive printing and checking; implemented by `sys.disable={'hygiene'}; sys.output='hush'`.
- Lines 66-67: Loop over correlation time; implemented by `parfor n=1:numel(tau_c)`.
- Lines 69-70: Localise parameter arrays; implemented by `locint=inter; locpar=parameters`.
- Lines 72-73: Set correlation time; implemented by `locint.tau_c={tau_c(n)}`.
- Lines 75-76: Spinach housekeeping; implemented by `spin_system=create(sys,locint)`.
- Lines 79-91: Relevant states; implemented by `locpar.coil=[state(spin_system,{'L+'},{2})+2*state(spin_system,{'L+','Lz'},{2,3}) state(spin_system,{'L+'},{2})-2*state(spin_system,{'L+','Lz'},{2,3}) state(spin_system,…`.
- Lines 95-96: Steady state simulation; implemented by `answer(:,n)=liquid(spin_system,@dnp_freq_scan,locpar,'esr')`.
- Lines 100-101: Plotting; implemented by `kfigure(); subplot(1,3,1); scale_figure([2.0 0.8])`.

### Control flow inferred from the code

- Line 67: `parfor` loop over `n=1:numel(tau_c)`.
