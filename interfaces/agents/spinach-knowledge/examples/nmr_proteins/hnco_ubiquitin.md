...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/nmr_proteins/hnco_ubiquitin.m`
- Signature: `hnco_ubiquitin()`
- Total lines: 82

## Purpose

Theoretical HNCO of human ubiquitin. It is assumed that only the backbone is 13C,15N-labelled. Calculation time: minutes, faster with a Tesla A100 GPU.
...
- Lines 17-18: Magnet field; implemented by `sys.magnet=11.7395`.
...
- Lines 38-39: Sequence parameters; implemented by `parameters.spins={'15N','13C','1H'}`.
- Lines 48-49: Simulation; implemented by `fid=liquid(spin_system,@hnco,parameters,'nmr')`.
- Lines 51-52: Apodisation; implemented by `fid.pos_pos=apodisation(spin_system,fid.pos_pos,{{'sqcos'},{'sqcos'},{'sqcos'}})`.
- Lines 57-58: F3 Fourier transform; implemented by `f3_pos_pos=fftshift(fft(fid.pos_pos,parameters.zerofill(3),3),3)`.
- Lines 63-64: Absorption part of F3 signal; implemented by `f3_pos=f3_pos_pos+conj(f3_neg_neg)`.
- Lines 67-68: F2 Fourier transform; implemented by `f3f2_pos=fftshift(fft(f3_pos,parameters.zerofill(2),2),2)`.
- Lines 71-72: Absorption part of F2 signal; implemented by `f3f2=f3f2_pos+conj(f3f2_neg)`.
- Lines 74-75: F1 Fourier transform; implemented by `spectrum=fftshift(fft(f3f2,parameters.zerofill(1),1),1)`.
- Lines 77-79: Plotting; implemented by `kfigure(); plot_3d(spin_system,real(spectrum),parameters, 10,[0.2 0.9 0.2 0.9],2,'positive')`.
...
- Lines 18: computes `sys.magnet` using `sys.magnet=11.7395`.
...
- Lines 39: computes `parameters.spins` using `parameters.spins={'15N','13C','1H'}`.
- Lines 40: computes `parameters.offset` using `parameters.offset=[-5900 22050 4100]`.
- Lines 41: computes `parameters.sweep` using `parameters.sweep=[2000 1600 2900]`.
- Lines 42: computes `parameters.npoints` using `parameters.npoints=[64 64 64]`.
