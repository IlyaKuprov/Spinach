% creates propagator for RF pulse with Bterm when relaxation is
% included
%
% input [arameters:
% parameters: structure containing simulation parameters
% expt: the Map containing the experimental parameters
% v_RF: RF frequency
% Hfree: acting Hamiltonian without RF term
% Iy: the Iy spin Operator dictionary
% t: pulse length
% Ni_ENDOR: number of ENDOR nuclei
% N_spinSys: number of spin systems
% R: the relaxation superoperator
% spin_system: Spinach spin system object
%
% output parameters:
% U: propagator for the pulse
%
% February 2024 A. Kehl (akehl@gwdg.de)



function U=kehl_rf_bterm_rlx(parameters,expt,v_RF,Hfree,Iy,t,Ni_ENDOR,N_spinSys,R,spin_system)
    if nargin<10
        spin_system=[];
    end


    % Check consistency
    grumble(parameters,expt,v_RF,Hfree,Iy,t,Ni_ENDOR,N_spinSys,R,spin_system);
    % Incrementation calculation for the RF pulse
    t_stepRF=1/(v_RF*parameters.N_stepRF);
    U_RF=eye(size(R));


    for ll=1:parameters.N_stepRF
        if N_spinSys==1
            HRF=Hfree;
            for mm=1:Ni_ENDOR
                HRF=HRF-2*(expt("oneN"))*Iy{mm}*cos(2*pi*v_RF*t_stepRF*(ll-1));
            end
        else
            HRF=Hfree-2*(expt("oneN"))*Iy{1}*cos(2*pi*v_RF*t_stepRF*(ll-1));
        end
        G=R-1i*full(hilb2liouv(sparse(HRF),'comm'));
        if isempty(spin_system)
            U_step=expm(G*t_stepRF);
        else
            U_step=full(propagator(spin_system,1i*sparse(G),t_stepRF));
        end
        U_RF=U_step*U_RF;
    end

    U=(U_RF^(t*v_RF));
end

function grumble(parameters,expt,v_RF,Hfree,Iy,t,Ni_ENDOR,N_spinSys,R,spin_system)
if ~isstruct(parameters)
    error('parameters must be a structure.');
end
if ~isa(expt,'containers.Map')
    error('expt must be a containers.Map object.');
end
if ~isnumeric(v_RF)
    error('v_RF must be numeric.');
end
if ~isnumeric(Hfree)
    error('Hfree must be numeric.');
end
if (~iscell(Iy))&&(~isnumeric(Iy))
    error('Iy must be numeric, or a cell array.');
end
if ~isnumeric(t)
    error('t must be numeric.');
end
if ~isnumeric(Ni_ENDOR)
    error('Ni_ENDOR must be numeric.');
end
if ~isnumeric(N_spinSys)
    error('N_spinSys must be numeric.');
end
if ~isnumeric(R)
    error('R must be numeric.');
end
if (~isstruct(spin_system))||(~isfield(spin_system,'bas'))||(~isfield(spin_system,'comp'))
    error('spin_system must be a Spinach spin system structure.');
end
end

