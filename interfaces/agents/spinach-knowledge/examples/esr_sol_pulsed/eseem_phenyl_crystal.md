...
- Lines 12-13: Spin system properties (vacuum DFT calculation); implemented by `options.no_xyz=1`.
...
- Lines 54-55: Plot the spectrum; implemented by `subplot(2,1,2); plot(ax,abs(spectrum))`.
...
- Lines 51-52: computes `ax` using `ax=linspace(-1/(parameters.timestep),1/(parameters.timestep), parameters.zerofill)*1e-6`.
