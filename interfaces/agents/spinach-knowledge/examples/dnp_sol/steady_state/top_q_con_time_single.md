...
- Lines 55-56: TOP loop count; implemented by `loop_counts=1:256`.
- Lines 58-59: Experiment parameters; implemented by `parameters.spins={'E','1H'}`.
- Lines 65-66: Over loop counts; implemented by `dnp_a=zeros(size(loop_counts),'like',1i)`.
- Lines 70-71: Localise parameters; implemented by `localpar=parameters`.
- Lines 73-74: Set the number of loops; implemented by `localpar.nloops=loop_counts(m)`.
- Lines 76-77: Parameter set A; implemented by `localpar.irr_powers=18e6`.
- Lines 83-84: Run the steady state simulation A; implemented by `dnp_a(m)=powder(spin_system,@topdnp_steady,localpar,'esr')`.

### Control flow inferred from the code

- Line 68: `parfor` loop over `m=1:numel(loop_counts)`.
