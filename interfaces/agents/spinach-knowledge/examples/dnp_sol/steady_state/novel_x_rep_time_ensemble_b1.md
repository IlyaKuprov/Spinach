...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/steady_state/novel_x_rep_time_ensemble_b1.m`
...
- Lines 42-43: B1 ensemble; implemented by `[b1,wb1]=gaussleg(14e6,16e6,5)`.
- Lines 45-46: Log spacing for rep. time; implemented by `rep_time=logspace(-4,-2,30)`.
- Lines 48-49: Preallocate equilibrium DNP value array; implemented by `dnp_noflip=zeros([numel(rep_time) numel(b1)],'like',1i)`.
...
- Line 87: `parfor` loop over `m=1:numel(rep_time)`.
...
- Lines 43: computes `[b1,wb1]` using `[b1,wb1]=gaussleg(14e6,16e6,5)`.
- Lines 46: computes `rep_time` using `rep_time=logspace(-4,-2,30)`.
- Lines 49: computes `dnp_noflip` using `dnp_noflip=zeros([numel(rep_time) numel(b1)],'like',1i)`.
- Lines 50: computes `dnp_flip` using `dnp_flip=zeros([numel(rep_time) numel(b1)],'like',1i)`.
- Lines 54: computes `inter.relaxation` using `inter.relaxation={'t1_t2'}`.
- Lines 55-56: computes `r1n_rate` using `r1n_rate=@(alp,bet,gam)r1n_dnp(sys.magnet,inter.temperature, 2.00230,1e-3,26,r_en,bet)`.
...
- Called routines detected from the main body: `cell2mat()`, `xyz()`, `gaussleg()`, `r1n_dnp()`, `create()`, `basis()`, `state()`, `rep_time()`, `dnp_noflip()`, `powder()`, `dnp_flip()`, `kfigure()`, `kylabel()`, `klegend()`, `kxlabel()`, `xlim()`.
