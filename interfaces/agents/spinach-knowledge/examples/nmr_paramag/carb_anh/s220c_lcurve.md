...
L-curves for the S220C mutant dataset for human carbonic anhydrase II. The system and the method are described in: A step-by-step tutorial is available here:
...
- Lines 20-21: Load experimental data; implemented by `load('s220c_expt.mat','expt_pcs','xyz','xyz_all')`.
- Lines 23-24: Load susceptibility tensor; implemented by `load('s220c_chi_eff.mat','chi')`.
...
- Lines 29: computes `parameters.box_cent` using `parameters.box_cent=[-16.0 -25.5 6.0]`.
...
- L-curves for the S220C mutant dataset for human carbonic anhydrase
