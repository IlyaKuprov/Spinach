...
- Lines 58-59: Simulation A -chirp; implemented by `parameters.pulse_pwr=2*pi*10e6*ones(100,1)`.
- Lines 63-64: Simulation B -fixed frequency; implemented by `parameters.pulse_pwr=2*pi*10e6*ones(100,1)`.
- Lines 68-69: Simulation C -reference; implemented by `parameters.pulse_pwr=zeros(100,1)`.
- Lines 73-74: Apodisation; implemented by `fid_a=apodisation(spin_system,fid_a,{{'exp',6}})`.
- Lines 78-79: Fourier transform; implemented by `spectrum_a=fftshift(fft(fid_a,parameters.zerofill))`.
- Lines 83-84: Plotting; implemented by `kfigure(); hold on`.
