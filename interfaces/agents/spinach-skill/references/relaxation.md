...
| spin-phonon (`rlx_phonon.m`, not yet an `inter.relaxation` theory) | Generalised Lindblad dissipator of Saito and Miyashita: a coupling operator, a phonon spectral density `I0*w^alpha`, and a bath temperature, evaluated in the eigenbasis of the current Hamiltonian | arguments of `rlx_phonon(spin_system,H,X,I0,alpha,T)`; driven by `experiments/pulsed_field.m` on every stair of a field profile | Pulsed-field magnetometry of molecular magnets; see `examples/giant_spin/case_studies` |
