...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/nmr_paramag/carb_anh/s166c_kuprov.m`
...
- Lines 20-21: Load experimental data; implemented by `load('s166c_expt.mat','expt_pcs','xyz','xyz_all')`.
- Lines 23-24: Load susceptibility tensor; implemented by `load('s166c_chi_eff.mat','chi')`.
...
- Lines 30: computes `parameters.box_cent` using `parameters.box_cent=[-14.0 -3.6 -11.0]`.
- Lines 31: computes `parameters.box_size` using `parameters.box_size=[ 20.0 20.0 20.0]`.
...
- Lines 43: computes `[source_cube,ranges]` using `[source_cube,ranges]=ipcs(parameters,n,0.21)`.
