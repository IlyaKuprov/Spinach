...
- Lines 10-11: Spin system; implemented by `[sys,inter,bas]=cyprinol()`.
...
- Lines 55-56: Simulation; implemented by `fid=liquid(subsystem,@hsqc,parameters,'nmr')`.
- Lines 58-59: Apodisation; implemented by `fid.pos=apodisation(spin_system,fid.pos,{{'sqcos'},{'sqcos'}})`.
- Lines 62-63: F2 Fourier transform; implemented by `f1_pos=fftshift(fft(fid.pos,parameters.zerofill(2),1),1)`.
- Lines 66-67: Form States signal; implemented by `fid=f1_pos+conj(f1_neg)`.
- Lines 69-70: F1 Fourier transform; implemented by `spectrum=spectrum+fftshift(fft(fid,parameters.zerofill(1),2),2)`.
