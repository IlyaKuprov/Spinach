...
Two-spin asymmetric magnetization flux problem. Calculation time: seconds.
...
- Lines 14: computes `inter.chem.flux_rate(1,2)` using `inter.chem.flux_rate(1,2)=5e2`.
...
- Lines 28: computes `parameters.rho0` using `parameters.rho0=2e3*state(spin_system,{'L+'},{1})+`.
