...
- Lines 10-11: Spin system properties; implemented by `[sys,inter]=strychnine({'1H','13C'})`.
...
- Lines 53-54: Simulation; implemented by `fid=liquid(subsystem,@hsqc,parameters,'nmr')`.
- Lines 56-57: Apodisation; implemented by `fid.pos=apodisation(spin_system,fid.pos,{{'sqcos'},{'sqcos'}})`.
- Lines 60-61: F2 Fourier transform; implemented by `f1_pos=fftshift(fft(fid.pos,parameters.zerofill(2),1),1)`.
- Lines 64-65: Form States signal; implemented by `fid=f1_pos+conj(f1_neg)`.
- Lines 67-68: F1 Fourier transform; implemented by `spectrum=spectrum+fftshift(fft(fid,parameters.zerofill(1),2),2)`.
- Lines 72-73: Plotting; implemented by `kfigure(); scale_figure([1.5 2.0])`.
...
- Lines 33: computes `parameters.decouple_f1` using `parameters.decouple_f1={'1H'}`.
- Lines 34: computes `parameters.decouple_f2` using `parameters.decouple_f2={'13C'}`.
