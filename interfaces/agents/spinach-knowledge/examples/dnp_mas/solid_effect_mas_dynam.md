...
- Lines 75-76: Thermal equilibrium state; implemented by `rho_eq=equilibrium(spin_system,hamiltonian(assume(spin_system,'labframe'),'left'))`.
- Lines 78-79: Rotor period trajectory; implemented by `nsteps=numel(H); rho=zeros(numel(rho_eq),nsteps,'like',1i); rho(:,1)=rho_eq`.
- Lines 86-87: Trajectory analysis; implemented by `kfigure(); trajan(spin_system,rho,'level_populations')`.

### Control flow inferred from the code

- Line 80: `for` loop over `n=2:nsteps`.
