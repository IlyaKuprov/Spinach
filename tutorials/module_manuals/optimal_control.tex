...
control structure included, with drifts, control operators, and offset
operators) to the parallel pool as a \code{parallel.pool.Constant} held
in \code{control.invariants}. The three heavy generator fields are then
removed from the client-side structure. A
...
worker-resident frozen problem (a fast local dereference), grafts the
current values of the \emph{live} fields --- states, timings, powers,
offsets, phase cycle, distortions, freeze mask, fidelity type, plotting
set, keyholes, and the trapezium commutator tables --- assembles its
...
\item[May be overwritten between optimisations] (the \emph{live} set):
  \code{rho\_init}, \code{rho\_targ} (same member counts),
  \code{pulse\_dt} (same length), \code{pwr\_levels},
  ...
  weights, and bounds. Overwrites take effect at the next objective
  evaluation --- this is how parameter sweeps reuse one
  \code{optimcon()} setup across many optimisations.
\item[Frozen] (changes require a fresh \code{optimcon()} call): the
  ...
  --- and the generators: \code{drifts}, \code{operators},
  \code{off\_ops}. The generator fields are physically absent from the
...
\code{carrier\_ops} + \code{carrier\_frq} & none & Bloch--Siegert
carrier data, absorbed together; reserved for future use --- no
consumer in the current kernel\\
...
excluded from optimisation; may not freeze everything\\
...
(timing), \code{cc\_comm} and \code{cc\_comm\_idx} (trapezium
control--control commutators and their zero table), \code{catalog} and
\code{ens\_sizes} (the ensemble case catalog and dimension sizes),
\code{invariants} (the worker-resident frozen problem,
a \code{parallel.pool.Constant}), the line search and regularisation
...
parsed forms of every option above. The fields \code{drifts},
\code{operators}, and \code{off\_ops} are \emph{absent} from the
