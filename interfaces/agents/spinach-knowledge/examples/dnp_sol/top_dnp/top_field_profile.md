...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/top_dnp/top_field_profile.m`
- Signature: `top_field_profile()`
- Total lines: 82

## Purpose

Field profile of a TOP DNP experiment. <I_z> after a fixed contact time is calculated as a function of electron pulse amplitude and offset. Further information in: Calculation time: minutes (a large powder grid is needed)
...
- Lines 68-69: Get the contact curve; implemented by `contact_curve=powder(spin_system,@topdnp,localpar,'esr')`.
...
- Lines 49: computes `parameters.pulse_dur` using `parameters.pulse_dur=10e-9`.
- Lines 50: computes `parameters.delay_dur` using `parameters.delay_dur=14e-9`.
- Lines 51: computes `parameters.nloops` using `parameters.nloops=300`.
