...
function [endor_amp,endor_amp_conv,x_coords,v_L]=kehl_tensor_sim(constants,spinSys,spinOps,expt,opt)
    ...
    if opt("Relax")==true
        ...
        endor_amp=kehl_tensor_rlx(constants,spinOps,spinSys,expt,opt,paramsENDOR,epr);
    else

        % starts the actual calculation
        endor_amp=kehl_tensor_calc(constants,spinOps,spinSys,expt,opt,paramsENDOR,epr);
    end
