...
base_energy=[-3;-1;2;5];
pert_mat=[ 0.0100  0.0020+0.0010i -0.0010         0.0005-0.0004i; ...
           0.0020-0.0010i -0.0070  0.0012+0.0006i -0.0008;        ...
          -0.0010  0.0012-0.0006i  0.0040         0.0015+0.0007i; ...
           0.0005+0.0004i -0.0008  0.0015-0.0007i -0.0020];
[vv_energy,vv_gen]=vvpert(base_energy,pert_mat,8);
exact_energy=sort(real(eig(diag(base_energy)+pert_mat,'vector')));
result=test_close(result,'vvpert eighth-order energy shifts',sort(vv_energy),...
                  exact_energy,1e-12,1e-12,...
                  'eighth-order Van Vleck perturbation theory tracks exact diagonalisation');
result=test_close(result,'vvpert higher-order anti-Hermitian generator',...
                  vv_gen+vv_gen',zeros(4),1e-13,1e-13,...
                  'the higher-order generator remains anti-Hermitian for a Hermitian perturbation');
