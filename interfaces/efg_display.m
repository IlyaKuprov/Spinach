...
function efg_display(props,atoms,scaling,conmatrix,options)
...
for n=idx
    ...
    % Get the current EFG or NQI tensor
    if isfield(props,'nqi')&&(~isempty(props.nqi{n}))
        disp(['EFG display: NQI tensor found for nucleus ' num2str(n)]); efg=props.nqi{n};
    elseif isfield(props,'efg')&&(~isempty(props.efg{n}))
        disp(['EFG display: EFG tensor found for nucleus ' num2str(n)]); efg=props.efg{n};
    else
        error(['EFG display: no NQI or EFG data found for nucleus ' num2str(n)]);
    end 

    % Eliminate the isotroic part
    if options.kill_iso
        efg=efg-eye(3)*trace(efg)/3;
    end
    ...
    switch options.style
        ...
        case 'ellipsoids'
            ...
            [eigvecs,eigvals]=eig(efg,'vector');
        ...
        case 'harmonics'
            ...
            [rank0,rank1,rank2]=mat2sphten(efg);
...
if ~isfield(props,'std_geom')
    error('props structure does not contain std_geom field.');
end
if ~isfield(props,'symbols')
    error('props structure does not contain symbols field.');
end
if iscell(atoms)
...
% Overheard at the EUROMAR Conference:
%
% "...it's just not the way Nature intended things 
% to be done - like communism, anal sex, and inver-
% se Laplace transform."
