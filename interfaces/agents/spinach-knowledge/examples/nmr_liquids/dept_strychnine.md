...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/nmr_liquids/dept_strychnine.m`
...
- Lines 34-35: Create the spin system structure; implemented by `spin_system=create(sys,inter)`.
- Lines 37-38: Generate isotopomers; implemented by `subsystems=dilute(spin_system,'13C')`.
- Lines 40-41: Preallocate the answer; implemented by `spectrum=zeros([parameters.zerofill 1],'like',1i)`.
...
- Lines 49-50: Simulation; implemented by `fid=liquid(subsystem,@dept,parameters,'nmr')`.
- Lines 52-53: Apodisation; implemented by `fid=apodisation(spin_system,fid,{{'exp',6}})`.
- Lines 55-56: Fourier transform; implemented by `spectrum=spectrum+fftshift(fft(fid,parameters.zerofill))`.
- Lines 60-61: Plotting; implemented by `kfigure(); parameters.spins={'13C'}`.
...
- Lines 32: computes `parameters.beta` using `parameters.beta=3*pi/4`.
- Lines 35: computes `spin_system` using `spin_system=create(sys,inter)`.
- Lines 38: computes `subsystems` using `subsystems=dilute(spin_system,'13C')`.
- Lines 41: computes `spectrum` using `spectrum=zeros([parameters.zerofill 1],'like',1i)`.
