...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_mas/cross_effect_mas_steady.m`
- Signature: `cross_effect_mas_steady()`
- Total lines: 112

## Purpose

A MAS DNP simulation performed as described in Fred Mentink- Vigier's paper (Spinach rotation conventions are different): Steady state rotor period simulation for a single crystal, computed using Newton-Raphson steady state solver. Calculation time: minutes
...
- Lines 68-69: Zeeman offset operators; implemented by `HzE=operator(spin_system,'Lz','E')`.
...
- Lines 79-80: Rotor period integration; implemented by `nsteps=numel(H); P=speye(size(H{1}))`.
- Lines 88-89: Steady state; implemented by `rho_st=steady(spin_system,P,[],'newton')`.
- Lines 91-92: Rotor period trajectory; implemented by `nsteps=numel(H); rho=zeros(numel(rho_st),nsteps,'like',1i); rho(:,1)=rho_st`.
- Lines 99-100: Trajectory analysis; implemented by `kfigure(); trajan(spin_system,rho,'level_populations')`.
- Lines 102-103: Thermal equilibrium state; implemented by `rho_eq=equilibrium(spin_system,hamiltonian(assume(spin_system,'labframe'),'left'))`.
- Lines 105-106: Enhancement factor; implemented by `Hz_dnp=mean(state(spin_system,'Lz','1H')'*rho)`.

### Control flow inferred from the code

- Line 82: `for` loop over `n=1:nsteps`.
- Line 93: `for` loop over `n=2:nsteps`.
