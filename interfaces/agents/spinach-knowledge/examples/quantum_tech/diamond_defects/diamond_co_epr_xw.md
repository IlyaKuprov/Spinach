...
- Lines 10-11: Set Co centre model parameters.; implemented by `co_params.orientation='111'`.
- Lines 14-15: Build the spin system.; implemented by `[sys,inter]=diamond_co(co_params)`.
...
- Lines 11: computes `co_params.orientation` using `co_params.orientation='111'`.
- Lines 12: computes `co_params.centre` using `co_params.centre='o4'`.
- Lines 15: computes `[sys,inter]` using `[sys,inter]=diamond_co(co_params)`.
...
- Lines 32: computes `parameters.int_tol` using `parameters.int_tol=1e-5`.
...
- Lines 39: computes `parameters.window` using `parameters.window=[0.2 0.45]`.
