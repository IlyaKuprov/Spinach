...
* **No bloat, no garbage:** You code must be minimalist. Do not leave any dead code, unused variables, or other redundant items in the functions you create or edit. Trivial helper functions are forbidden. Never create a separate function or a helper that is only called once. Do not create new features where existing *Spinach* features may be used. Never implement any option or structure you have not been directly asked to implement. Never add anything that does not need to be added. The use of `varargin` and `varargout` is discouraged: write a fixed signature wherever one is possible. They are permitted only where *MATLAB* itself dispatches a variadic call, for example the `horzcat`, `vertcat`, and `cat` overloads, which receive every operand of `[A B C]` in a single call and therefore cannot be written with a fixed signature. Avoid object-oriented nonsense and use strict functional programming everywhere.
