%KEHL_OPERATOR_BASIS Cached operators for Kehl ENDOR kernels.
%
%   OPS=KEHL_OPERATOR_BASIS(SPIN_SYSTEM,N_ENDOR,N_SPIN_SYSTEMS) returns a
%   cached structure containing electron and nuclear spin operators, their
%   legacy diagonal-frame forms, and the electron Lz initial state used by
%   the Kehl kernels.
%
%   Inputs:
%
%      SPIN_SYSTEM     - reduced Spinach spin system for Kehl kernels.
%
%      N_ENDOR         - number of ENDOR nuclei represented by the context.
%
%      N_SPIN_SYSTEMS  - number of separated spin systems in legacy mode.
%
%   Output:
%
%      OPS             - cached operator structure.
%
...
% Precompute the state used when no Boltzmann correction is requested
ops.rho_z=-full(state(spin_system,'Lz',1));
