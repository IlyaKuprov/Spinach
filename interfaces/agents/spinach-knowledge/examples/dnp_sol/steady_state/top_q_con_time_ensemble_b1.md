...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/steady_state/top_q_con_time_ensemble_b1.m`
...
- Lines 42-43: TOP loop count; implemented by `loop_counts=1:256`.
- Lines 45-47: Relaxation rates, distance and orientation dependence provided using a function handle; implemented by `inter.relaxation={'t1_t2'}`.
- Lines 55-56: Spinach housekeeping; implemented by `spin_system=create(sys,inter)`.
- Lines 59-60: Detect the proton; implemented by `parameters.coil=state(spin_system,'Lz','1H')`.
- Lines 62-63: Experiment parameters; implemented by `parameters.spins={'E','1H'}`.
- Lines 69-70: B1 ensemble A; implemented by `[b1,wb1]=gaussleg(10e6,20e6,5)`.
- Lines 72-73: Preallocate equilibrium DNP value array; implemented by `dnp_a=zeros([numel(loop_counts) numel(b1)],'like',1i)`.
- Lines 75-76: Over B1 fields; implemented by `for k=1:numel(b1)`.
- Lines 78-79: Set electron nutation frequency; implemented by `parameters.irr_powers=b1(k)`.

### Control flow inferred from the code

- Line 76: `for` loop over `k=1:numel(b1)`.
- Line 82: `parfor` loop over `m=1:numel(loop_counts)`.
- Line 113: `for` loop over `k=1:numel(b1)`.
- Line 119: `parfor` loop over `m=1:numel(loop_counts)`.
...
- Lines 43: computes `loop_counts` using `loop_counts=1:256`.
- Lines 47: computes `inter.relaxation` using `inter.relaxation={'t1_t2'}`.
- Lines 48-49: computes `r1n_rate` using `r1n_rate=@(alp,bet,gam)r1n_dnp(sys.magnet,inter.temperature, 2.00230,1e-3,52,r_en,bet)`.
- Lines 50: computes `inter.r1_rates` using `inter.r1_rates={1e3 r1n_rate}`.
- Lines 51: computes `inter.r2_rates` using `inter.r2_rates={200e3 50e3}`.
- Lines 52: computes `inter.rlx_keep` using `inter.rlx_keep='diagonal'`.
- Lines 53: computes `inter.equilibrium` using `inter.equilibrium='dibari'`.
...
- Called routines detected from the main body: `cell2mat()`, `xyz()`, `r1n_dnp()`, `create()`, `basis()`, `state()`, `gaussleg()`, `loop_counts()`, `dnp_a()`, `powder()`, `dnp_b()`, `kfigure()`, `kylabel()`, `klegend()`, `kxlabel()`, `savefig()`.
