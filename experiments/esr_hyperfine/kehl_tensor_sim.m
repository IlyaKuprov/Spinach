...
function [endor_amp,endor_amp_conv,x_coords,v_L]=kehl_tensor_sim(constants,spinSys,spinOps,expt,parameters)
    ...
    if parameters.Relax==true
        warning('Only T2 relaxation is supported for the tensor calculation.');

        % starts the actual calculation with relaxation
        endor_amp=kehl_tensor_rlx(constants,spinOps,spinSys,expt,parameters,paramsENDOR,epr);
    else

        % starts the actual calculation
        endor_amp=kehl_tensor_calc(constants,spinOps,spinSys,expt,parameters,paramsENDOR,epr);
    end
