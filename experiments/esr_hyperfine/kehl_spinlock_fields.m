...
function expt=kehl_spinlock_fields(constants,expt,set,values)
    if nargin<4
        values=[];
    end
    ...
    % setting the field strength values as needed for the Spinlock ENDOR experiment
    t=expt("t");
    if set==false
        if size(values,1)==3
            expt("SL")=values(1)*2*pi*1e6;
            expt("oneE")=values(2)*2*pi*1e6;
            expt("oneN")=values(3)*2*pi*1e3;
