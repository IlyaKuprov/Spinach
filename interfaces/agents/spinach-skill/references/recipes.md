...
that is open when it runs and records that pool's identity; `ensemble` refuses
any other pool, even one with the same number of workers, and refuses a call
from inside a worker: keep the same pool object from `optimcon` through
`fmaxnewton` and any later `ensemble` evaluation, and re-run `optimcon` after
deleting or restarting the pool. Sweeping `pwr_levels` and `offsets` makes B1 inhomogeneity and
