...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/nmr_overtone/mas_boron_3.m`
...
- Lines 24: computes `inter.damp_rate` using `inter.damp_rate=1000`.
...
- Lines 37: computes `parameters.grid` using `parameters.grid='rep_2ang_200pts_oct'`.
- Lines 38: computes `parameters.sweep` using `parameters.sweep=[-200e3 200e3]`.
- Lines 39: computes `parameters.npoints` using `parameters.npoints=4096`.
- Lines 40: computes `parameters.zerofill` using `parameters.zerofill=4096`.
