...
- Lines 28-29: Coherence order to convert, 3 or 5; implemented by `mq_order=5`.
- Lines 31-32: 400 MHz magnet; implemented by `sys.magnet=2*pi*400e6/spin('1H')`.
- Lines 35-36: Quadrupolar coupling and shielding anisotropy; implemented by `inter.coupling.matrix{1,1}=eeqq2nqi(3.0e6,1.0,5/2,[0 0 0])`.
- Lines 40-41: Hilbert space formalism; implemented by `bas.formalism='zeeman-hilb'`.
- Lines 44-45: Spinach housekeeping; implemented by `spin_system=create(sys,inter)`.
- Lines 49-50: Rotor phase resolved drift Hamiltonians; implemented by `parameters.spins={'27Al'}`.
- Lines 58-59: Initial state, symmetric MQ coherence between m=+mq_order/2 and m=-mq_order/2; implemented by `rho_init=zeros(6); rho_init(3.5-mq_order/2,3.5+mq_order/2)=1`.
- Lines 62-63: Target state, population difference across the central transition; implemented by `rho_targ=diag([0 0 1 -1 0 0]); rho_targ=rho_targ/norm(rho_targ,'fro')`.
...
- Lines 29: computes `mq_order` using `mq_order=5`.
- Lines 32: computes `sys.magnet` using `sys.magnet=2*pi*400e6/spin('1H')`.
- Lines 33: computes `sys.isotopes` using `sys.isotopes={'27Al'}`.
- Lines 36: computes `inter.coupling.matrix{1,1}` using `inter.coupling.matrix{1,1}=eeqq2nqi(3.0e6,1.0,5/2,[0 0 0])`.
- Lines 37: computes `inter.zeeman.eigs` using `inter.zeeman.eigs={[-5 -5 10]}`.
- Lines 38: computes `inter.zeeman.euler` using `inter.zeeman.euler={[0 0 0]}`.
...
- Lines 50: computes `parameters.spins` using `parameters.spins={'27Al'}`.
- Lines 51: computes `parameters.axis` using `parameters.axis=[sqrt(2/3) 0 sqrt(1/3)]`.
- Lines 52: computes `parameters.grid` using `parameters.grid='rep_2ang_200pts_sph'`.
- Lines 53: computes `parameters.n_ticks` using `parameters.n_ticks=160`.
- Lines 54: computes `parameters.n_phases` using `parameters.n_phases=80`.
- Lines 55: computes `parameters.n_slices` using `parameters.n_slices=480`.
- Lines 56: computes `control.drifts` using `control.drifts=mqmas_drifts(spin_system,parameters)`.
- Lines 59: computes `rho_init` using `rho_init=zeros(6); rho_init(3.5-mq_order/2,3.5+mq_order/2)=1`.
- Lines 63: computes `rho_targ` using `rho_targ=diag([0 0 1 -1 0 0]); rho_targ=rho_targ/norm(rho_targ,'fro')`.
