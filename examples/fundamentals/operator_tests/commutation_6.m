% Delicate commutation and product relations for Pauli and
...
for mult=[4 6 8]
    ...
    if max([err_comm err_lad_1 err_lad_2 err_lad_3 err_prod])>tol
        error('Central transition commutation test FAILED.');
    end

end
disp('Central transition commutation test PASSED.');
...
for mult=[4 6 8]
    for type={'x','y','z','+','-'}
        ...
        if norm(C-C_rec,'fro')>tol
            error('Central transition IST expansion test FAILED.');
        end

    end
end
disp('Central transition IST expansion test PASSED.');
