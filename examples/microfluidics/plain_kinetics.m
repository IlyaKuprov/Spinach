...
% with inverted cyclopentadiene magnetisation 
...
for n=1:nsteps
    ...
    report(spin_system,['evolution time step ' int2str(n) ...
                        '/' int2str(nsteps)]);
...
coils=[state(spin_system,{'Lz'},{6})  ...
       state(spin_system,{'Lz'},{9})  ...
       state(spin_system,{'Lz'},{11}) ...
       state(spin_system,{'Lz'},{13}) ...
       state(spin_system,{'Lz'},{20}) ...
       state(spin_system,{'Lz'},{22})];
x=real((coils'*coils)\(coils'*traj));

% Plot concentration-weighted observables, excluding solvent
subplot(1,2,2); plot(time_axis,x([1 3 5],:),'LineStyle','-');
hold on;        plot(time_axis,x([2 4 6],:),'LineStyle','--');
xlim tight; ylim([-1.3e-5 3e-5]); kgrid; kxlabel('time, seconds'); 
kylabel('$\langle L_{\mathrm{Z}} \rangle$, mol/L');
klegend({'cyclopentadiene, CH$_{2}$ protons',...
         'endo-NBCN, CH$_{2}$ from cyclopentadiene',...
         'exo-NBCN, CH$_{2}$ from cyclopentadiene',...
         'acrylonitrile, CH proton (inverted)', ...
         'endo-NBCN, CH from acrylonitrile',...
         'exo-NBCN, CH from acrylonitrile'},...
         'Location','northeast'); drawnow;
