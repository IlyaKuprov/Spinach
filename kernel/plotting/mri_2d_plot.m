...
elseif strcmp(method,'k-space')
    if (~isnumeric(mri))||(~ismatrix(mri))
        error('mri must be a numeric matrix.');
    end
end
if ismember(method,{'image','k-space'})
    ...
    if (~isnumeric(parameters.ro_grad_amp))||(~isreal(parameters.ro_grad_amp))||...
       ...
       (parameters.ro_grad_amp==0)
        error('parameters.ro_grad_amp should be a finite non-zero real scalar.');
    end    
    ...
    if (~isnumeric(parameters.pe_grad_amp))||(~isreal(parameters.pe_grad_amp))||...
       ...
       (parameters.pe_grad_amp==0)
        error('parameters.pe_grad_amp should be a finite non-zero real scalar.');
    end
