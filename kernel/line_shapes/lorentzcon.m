...
elseif (offs(2)-offs(1)<sim_tol)
    ...
    y=(2*ampl/pi)*((x-offs(2))/((offs(1)-offs(2))*(offs(1)-offs(2)))).*atan2(offs(1)-x,gam)+...
      (2*ampl/pi)*((offs(2)-x)/((offs(1)-offs(2))*(offs(1)-offs(2)))).*atan2(offs(2)-x,gam)+...
...
elseif (offs(3)-offs(2)<sim_tol)
    ...
    % Lorentzian convolution with a right-angle triangle
    y=(2*ampl/pi)*((offs(1)-x)/((offs(1)-offs(2))*(offs(1)-offs(2)))).*atan2(offs(1)-x,gam)+...
      (2*ampl/pi)*((x-offs(1))/((offs(1)-offs(2))*(offs(1)-offs(2)))).*atan2(offs(2)-x,gam)+...
      ((ampl*gam/pi)/((offs(1)-offs(2))*(offs(1)-offs(2))))*log(((offs(2)-x).^2+gam^2)./((offs(1)-x).^2+gam^2));
 
else

    % Lorentzian convolution with a general triangle
    y=(2*ampl/pi)*((offs(1)-x)/((offs(1)-offs(2))*(offs(1)-offs(3)))).*atan2(offs(1)-x,gam)+...
      (2*ampl/pi)*((x-offs(2))/((offs(1)-offs(2))*(offs(2)-offs(3)))).*atan2(offs(2)-x,gam)+...
      (2*ampl/pi)*((x-offs(3))/((offs(1)-offs(3))*(offs(3)-offs(2)))).*atan2(offs(3)-x,gam)+...
      ((ampl*gam/pi)/((offs(1)-offs(2))*(offs(1)-offs(3))))*log(((offs(2)-x).^2+gam^2)./((offs(1)-x).^2+gam^2))+...
      ((ampl*gam/pi)/((offs(1)-offs(3))*(offs(3)-offs(2))))*log(((offs(3)-x).^2+gam^2)./((offs(2)-x).^2+gam^2));
