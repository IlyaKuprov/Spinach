% Simulation of T1e dependent XiX DNP optimization of repetition time in 
...
for j=1:numel(T1e)
    col=char(Color(j));
    xix_rep_time_ensemble_r(T1e(j),col)
    legend('10 ms','3 ms','1 ms','0.3 ms','0.1 ms','location','southeast')
end

savefig(gcf,'xix_rep_time_ensemble_r_T1e.fig');


function xix_rep_time_ensemble_r(T1e,col)
...
grid on; xlim([0 2]); ylim([0 1.7e-3]); hold on
