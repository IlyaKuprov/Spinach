...
- Lines 63-64: Simulation; implemented by `fid=liquid(subsystem,@ct_hsqc,parameters,'nmr')`.
- Lines 66-67: Apodisation; implemented by `fid.pos=apodisation(spin_system,fid.pos,{{'sqcos'},{'sqcos'}})`.
- Lines 70-71: F2 Fourier transform; implemented by `f1_pos=fftshift(fft(fid.pos,parameters.zerofill(2),1),1)`.
- Lines 74-75: Form States signal; implemented by `fid=f1_pos+conj(f1_neg)`.
- Lines 77-78: F1 Fourier transform; implemented by `spectrum=spectrum+fftshift(fft(fid,parameters.zerofill(1),2),2)`.
