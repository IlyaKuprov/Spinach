...
fsparse_args=loaded.mex_calls{4};
spsort_args=loaded.mex_calls{5};
spunicols_args=loaded.mex_calls{6};
...
fsparse_args_cmp=strrep(strrep(fsparse_args,'/',filesep),'\',filesep);
...
fsparse_out=fsparse_args_cmp{find(strcmp(fsparse_args,'-outdir'),1)+1};
...
result=test_true(result,'compile_mex compiler calls',...
                 ...
                 any(strcmp(fsparse_args_cmp,fullfile(scratch,'kernel','indexing','fsparse.cpp')))&&...
                 ...
                 strcmp(fsparse_out,fullfile(scratch,'kernel','indexing'))&&...
                 strcmp(spsort_out,fullfile(scratch,'kernel','indexing'))&&...
                 strcmp(spunicols_out,fullfile(scratch,'kernel','indexing')),...
                 'compile_mex must compile the six listed MEX sources into their source directories with the interleaved-complex API');
...
result=test_true(result,'compile_mex source confinement',...
                 ...
                 contains(compile_src,'kernel/indexing/fsparse.cpp')&&...
...
for n=1:numel(openmp_markers)
    no_openmp=no_openmp&&~contains(spsort_src,openmp_markers{n})&&...
              ~contains(spunicols_src,openmp_markers{n});
end
result=test_true(result,'compile_mex no OpenMP rigging',no_openmp,...
                 'spsortrows and spunicols source paths must not depend on OpenMP');
