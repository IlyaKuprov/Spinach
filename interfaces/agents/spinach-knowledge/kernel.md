...
| `kernel/optimcon/ens_block.m` | `[traj,fid,grad,hess]=ens_block(spin_system,drifts,control,block,waveform,n_outputs)` | Fidelity, gradient, and Hessian contributions of one block of ensemble cases, evaluated on the parallel pool worker that h | 191 |
