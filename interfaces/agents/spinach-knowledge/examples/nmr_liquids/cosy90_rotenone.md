...
- Lines 55-56: Sequence parameters; implemented by `parameters.angle=pi/2`.
- Lines 64-65: Spinach housekeeping; implemented by `spin_system=create(sys,inter)`.
- Lines 68-69: Simulation; implemented by `fid=liquid(spin_system,@cosy,parameters,'nmr')`.
- Lines 71-72: Apodisation; implemented by `fid=apodisation(spin_system,fid,{{'cos'},{'cos'}})`.
- Lines 74-76: Fourier transform; implemented by `spectrum=fftshift(fft2(fid,parameters.zerofill(2), parameters.zerofill(1)))`.
