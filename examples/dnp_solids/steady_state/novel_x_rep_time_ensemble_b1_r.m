...
function novel_x_rep_time_ensemble_b1_r()
...
dnp_noflip=zeros([numel(rep_time) numel(r) numel(b1)],'like',1i);
dnp_flip=zeros([numel(rep_time) numel(r) numel(b1)],'like',1i);
...
for n=1:numel(r)  
    ...
    for k=1:numel(b1)     
        ...
        parfor m=1:numel(rep_time)
            ...
            dnp_noflip(m,n,k)=powder(spin_system,@noveldnp_steady,localpar,'esr');
            ...
            % Run the steady state simulation
            dnp_flip(m,n,k)=powder(spin_system,@noveldnp_steady,localpar,'esr');
...
dnp_noflip=sum(dnp_noflip.*reshape(wb1,[1 1 numel(wb1)]),3)/sum(wb1);
dnp_flip=sum(dnp_flip.*reshape(wb1,[1 1 numel(wb1)]),3)/sum(wb1);
...
% Save for later
savefig(gcf,'novel_x_rep_time_ensemble_b1_r.fig');
save('novel_x_rep_time_ensemble_b1_r.mat');
