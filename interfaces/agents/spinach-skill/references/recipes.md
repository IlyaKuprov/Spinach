...
For cooperative phase control, `grape_coop` retains the requested primary fidelity and uses real-overlap auxiliary derivatives for its squared-impurity penalty, including with nonunit targets. Zero real auxiliary overlaps and zero impurity gradients are valid engine outputs. `fmaxnewton` checks the initial assembled gradient on unfrozen coordinates for all four methods, but `grape_coop` provides no Hessian and supports only `lbfgs` and `rbfgs` optimisation; do not reject individual auxiliary contributions. The initial gradient norm must be at least `1e-6`. Terminal objectives without trajectory penalties also require primary transfer magnitude of at least `1e-6`: `grape_coop` reports the pre-impurity value in `traj_data{1}{1}.primary_fid`, so callback adapters cannot confuse a cancelling composite score with zero primary transfer. Objective-only calls with `max_iter=0` do not apply these initial-guess checks.
