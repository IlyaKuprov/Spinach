...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/nmr_proteins/hncaco_gb1.m`
- Signature: `hncaco_gb1()`
- Total lines: 83

## Purpose

Simulated HNCACO spectrum of GB1 protein. It is assumed that only the backbone is 13C,15N-labelled. Calculation time: minutes, faster with a Tesla A100 GPU.
...
- Lines 38-39: Sequence parameters; implemented by `parameters.J_nh=92`.
- Lines 49-50: Simulation; implemented by `fid=liquid(spin_system,@hncaco,parameters,'nmr')`.
- Lines 52-53: Apodisation; implemented by `fid.pos_pos=apodisation(spin_system,fid.pos_pos,{{'sqcos'},{'sqcos'},{'sqcos'}})`.
- Lines 58-59: F3 Fourier transform; implemented by `f3_pos_pos=fftshift(fft(fid.pos_pos,parameters.zerofill(3),3),3)`.
- Lines 64-65: Absorption part of F3 signal; implemented by `f3_pos=f3_pos_pos+1i*conj(f3_neg_neg)`.
- Lines 68-69: F2 Fourier transform; implemented by `f3f2_pos=fftshift(fft(f3_pos,parameters.zerofill(2),2),2)`.
- Lines 72-73: Absorption part of F2 signal; implemented by `f3f2=f3f2_pos+1i*conj(f3f2_neg)`.
- Lines 75-76: F1 Fourier transform; implemented by `spectrum=fftshift(fft(f3f2,parameters.zerofill(1),1),1)`.
- Lines 78-80: Plotting; implemented by `kfigure(); plot_3d(spin_system,real(spectrum),parameters, 10,[0.2 0.9 0.2 0.9],2,'positive')`.
...
- Lines 39: computes `parameters.J_nh` using `parameters.J_nh=92`.
- Lines 40: computes `parameters.T` using `parameters.T=25e-3`.
- Lines 41: computes `parameters.delta2` using `parameters.delta2=3e-3`.
- Lines 42: computes `parameters.spins` using `parameters.spins={'15N','13C','1H'}`.
