...
- Lines 10-11: Spin system properties (vacuum DFT calculation); implemented by `options.min_j=1.0`.
...
- Lines 49-50: Simulation; implemented by `fid=liquid(spin_system,@noesy,parameters,'nmr')`.
- Lines 52-53: Apodisation; implemented by `fid.cos=apodisation(spin_system,fid.cos,{{'sqcos'},{'sqcos'}})`.
- Lines 56-57: F2 Fourier transform; implemented by `f1_cos=real(fftshift(fft(fid.cos,parameters.zerofill(2),1),1))`.
- Lines 60-61: States signal; implemented by `f1_states=f1_cos-1i*f1_sin`.
- Lines 63-64: F1 Fourier transform; implemented by `spectrum=fftshift(fft(f1_states,parameters.zerofill(1),2),2)`.
...
- Lines 21: computes `bas.prox_level` using `bas.prox_level=3`.
- Lines 24: computes `inter.relaxation` using `inter.relaxation={'redfield'}`.
- Lines 25: computes `inter.equilibrium` using `inter.equilibrium='IME'`.
...
- Lines 28: computes `inter.tau_c` using `inter.tau_c={200e-12}`.
...
- Lines 41: computes `parameters.offset` using `parameters.offset=800`.
