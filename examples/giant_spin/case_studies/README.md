...
Spinach reproductions of the simulation figures of the qdmag paper (Liu, Chen, Cupo, Fry, Cheng, arXiv:2609.16352), which describes non-equilibrium magnetisation of molecular magnets under a time-varying field with spin-phonon relaxation in the generalised Lindblad form of Saito and Miyashita. Every script uses the paper's own Hamiltonian parameters, temperatures, spectral density, sweep profiles, and stair widths; the kernel functions involved are `stevens.m` and `stev2sph.m` (crystal field to rank 12 through `inter.giant`), `rlx_phonon.m` (the spin-phonon dissipator; its thermally dressed coupling operator is rebuilt on every stair and applied as Hilbert space matrix products), and `pulsed_field.m` (the experiment, run under `crystal` or `powder`).
