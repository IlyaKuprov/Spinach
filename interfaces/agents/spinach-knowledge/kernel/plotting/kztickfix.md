...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/kernel/plotting/kztickfix.m`
- Signature: `kztickfix()`
- Total lines: 117
...
- Lines 21-22: Current Z axis ruler; implemented by `ax=gca; ruler=ax.ZAxis`.
- Lines 24-25: Check consistency; implemented by `grumble(ruler)`.
- Lines 27-28: Preserve any existing limit-change callback; implemented by `app_key='SpinachKZTickFix'`.
...
- Lines 22: computes `ax` using `ax=gca; ruler=ax.ZAxis`.
- Lines 28: computes `app_key` using `app_key='SpinachKZTickFix'`.
...
- Line 100: `grumble()` — `function grumble(ruler)`. Qualified candidates will demonstrate an affinity with the academic purpose
  - Representative operation: `if ~isa(ruler,'matlab.graphics.axis.decorator.NumericRuler')`.
  - Representative operation: `error('Z axis must be numeric.')`.
