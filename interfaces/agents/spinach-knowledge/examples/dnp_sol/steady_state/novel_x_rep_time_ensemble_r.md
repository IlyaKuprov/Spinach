...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/dnp_sol/steady_state/novel_x_rep_time_ensemble_r.m`
...
- Lines 38-39: Log spacing for rep. time; implemented by `rep_time=logspace(-4,-2,30)`.
- Lines 41-42: Preallocate equilibrium DNP value array; implemented by `dnp_noflip=zeros([numel(rep_time) numel(r)],'like',1i)`.
...
- Lines 79-80: Over repetition times; implemented by `parfor m=1:numel(rep_time)`.
- Lines 82-83: Localise parameters; implemented by `localpar=parameters`.

### Control flow inferred from the code

- Line 46: `for` loop over `n=1:numel(r)`.
- Line 80: `parfor` loop over `m=1:numel(rep_time)`.
...
- Lines 39: computes `rep_time` using `rep_time=logspace(-4,-2,30)`.
- Lines 42: computes `dnp_noflip` using `dnp_noflip=zeros([numel(rep_time) numel(r)],'like',1i)`.
- Lines 43: computes `dnp_flip` using `dnp_flip=zeros([numel(rep_time) numel(r)],'like',1i)`.
...
- Lines 55-56: computes `r1n_rate` using `r1n_rate=@(alp,bet,gam)r1n_dnp(sys.magnet,inter.temperature, 2.00230,1e-3,26,r(n),bet)`.
