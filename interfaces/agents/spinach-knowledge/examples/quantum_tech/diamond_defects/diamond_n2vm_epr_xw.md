...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/quantum_tech/diamond_defects/diamond_n2vm_epr_xw.m`
- Signature: `diamond_n2vm_epr_xw()`
- Total lines: 68

## Purpose

Field-swept powder EPR spectra of an N2V-centre in diamond at X and W bands. Calculation time: seconds.
...
- Lines 10-11: Set N2V-centre parameters; implemented by `n2v_params.orientation='111'`.
- Lines 15-16: Build the spin system; implemented by `[sys,inter]=diamond_n2vm(n2v_params)`.
...
- Lines 29-30: EPR sim parameters; implemented by `parameters.spins={'E'}`.
- Lines 38-39: Set X-band parameters; implemented by `parameters.mw_freq=9.755e9`.
...
- Lines 11: computes `n2v_params.orientation` using `n2v_params.orientation='111'`.
- Lines 12: computes `n2v_params.nitrogen` using `n2v_params.nitrogen='15N'`.
- Lines 13: computes `n2v_params.include_13c` using `n2v_params.include_13c=false`.
- Lines 16: computes `[sys,inter]` using `[sys,inter]=diamond_n2vm(n2v_params)`.
...
- Lines 30: computes `parameters.spins` using `parameters.spins={'E'}`.
- Lines 31: computes `parameters.grid` using `parameters.grid='rep_2ang_100pts_sph'`.
- Lines 32: computes `parameters.fwhm` using `parameters.fwhm=0.00003`.
- Lines 33: computes `parameters.int_tol` using `parameters.int_tol=0.1`.
...
- Lines 39: computes `parameters.mw_freq` using `parameters.mw_freq=9.755e9`.
- Lines 40: computes `parameters.window` using `parameters.window=[0.347 0.349]`.
...
- Called routines detected from the main body: `diamond_n2vm()`, `create()`, `basis()`, `fieldsweep()`, `kfigure()`, `scale_figure()`, `subplot()`, `kxlabel()`, `kylabel()`, `ktitle()`.
