...
- Lines 120-121: Get the current tensor; implemented by `hfc=props.hfc.full.matrix{n}`.
...
- Lines 134-135: Diagonalize the tensor; implemented by `[eigvecs,eigvals]=eig(hfc,'vector')`.
...
- Lines 121: computes `hfc` using `hfc=props.hfc.full.matrix{n}`.
- Lines 135: computes `[eigvecs,eigvals]` using `[eigvecs,eigvals]=eig(hfc,'vector')`.
