...
- Source: `/home/kuprov/.openclaw/workspace/Spinach/examples/optimal_control/magic_pulse_cart.m`
...
- Lines 56-57: Get the drift Hamiltonian; implemented by `D=hamiltonian(assume(spin_system,'nmr'))`.
- Lines 59-60: Define control parameters; implemented by `control.isotopes={'13C'}`.
- Lines 73-74: Plotting options; implemented by `control.plotting={'xy_controls','robustness','spectrogram'}`.
- Lines 76-77: Spinach housekeeping; implemented by `spin_system=optimcon(spin_system,control)`.
- Lines 79-80: Initial guess; implemented by `guess=(1/4)*ones(2,40)`.
- Lines 82-83: GRAPE optimisation; implemented by `xy_profile=fmaxnewton(spin_system,@grape_xy,guess)`.
- Lines 85-86: Extract the waveform; implemented by `xy_profile=mean(control.pwr_levels)*xy_profile`.
...
- Lines 62: computes `control.drifts` using `control.drifts={{D}}`.
...
- Called routines detected from the main body: `num2cell()`, `create()`, `basis()`, `state()`, `operator()`, `hamiltonian()`, `assume()`, `optimcon()`, `fmaxnewton()`, `xy_profile()`, `shaped_pulse_xy()`, `liquid()`, `apodisation()`, `fftshift()`, `figure()`, `subplot()`.
