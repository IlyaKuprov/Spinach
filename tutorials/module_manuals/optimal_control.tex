...
Two trajectory-dependent terms may be added to the objective, both
evaluated on the workers from the forward trajectory and differentiated
through costates that ride on the backward sweep. With
\code{control.fid\_type='average'} the functional above is applied to
the overlap at every pulse node $n=1,\ldots,N$ and averaged, instead of
being taken at the last node (\code{'terminal'}, the default); this
rewards early arrival at the target. With \code{control.traj\_pen}, a
cell array of Hermitian operators (square matrices in Hilbert space,
state-shaped vectors in Liouville space), the expectation value of their
sum along the trajectory is averaged over the same nodes and subtracted
from the fidelity; a projector onto leakage levels is the typical use.
The penalty has no weight of its own --- scale the operators. Neither
term has an analytic Hessian, and neither is available with the
stroboscopic steady state; trajectory penalties are also not available
with phase cycles or with \code{grape\_coop()}.
...
permitted when distortions are present (\code{lbfgs}, \code{rbfgs});
analytic Hessians of composed distortion chains are not implemented.
...
\item[May be overwritten between optimisations] (the \emph{live} set):
  ...
  \code{fidelity}, \code{fid\_type}, \code{traj\_pen}, \code{plotting},
  \code{keyholes}, penalties, weights, bounds, and the prefix and
  suffix functions. The live
...
\code{lbfgs} or \code{rbfgs}, and combine freely with all other ensemble dimensions ---
...
\code{fid\_type} & \code{'terminal'} & \code{'terminal'} takes the
fidelity at the last pulse node, \code{'average'} averages it over all
pulse nodes (Section~\ref{sec:fidelityfun}); \code{'average'} refuses
Hessian-based methods and the steady state\\
\code{traj\_pen} & \code{\{\}} & cell array of Hermitian operators
(matrices in Hilbert space, state-shaped vectors in Liouville space)
whose summed expectation value, averaged over the pulse nodes, is
subtracted from the fidelity (Section~\ref{sec:fidelityfun}); refuses
Hessian-based methods, the steady state, phase cycles, and
\code{grape\_coop()}\\
...
handles with Jacobians; requires \code{lbfgs} or \code{rbfgs}\\
...
\item[\code{[traj,fid,grad,hess]=ensemble(waveform,spin\_system)}] ---
  the parallel ensemble average of the fidelity, including the
  trajectory penalty when \code{traj\_pen} is set, but none of the
  amplitude penalties; called by the wrappers, callable directly.
\item[\code{tgrape()}] ---
...
keyholes                 & yes & no\\
waveform distortions     & LBFGS/RBFGS only & LBFGS/RBFGS only\\
